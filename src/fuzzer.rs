...
impl FuzzTask {
    fn random(rng: &mut impl Rng) -> Self {
        Self {
            ...
            session_id: if rng.gen_bool(0.8) {
                Some(random_string(rng))
            } else {
                None
            },
...
impl FuzzTaskStatus {
    fn random(rng: &mut impl Rng) -> Self {
        Self {
            ...
            message: if rng.gen_bool(0.5) {
                Some(FuzzMessage::random(rng))
            } else {
                None
            },
    ...
    fn random(rng: &mut impl Rng) -> Self {
        match rng.gen_range(0..=3) {
            ...
            1 => FuzzPart::File(
                random_string(rng),
                if rng.gen_bool(0.5) {
                    Some(format!("base64:{}", random_string(rng)))
                } else {
                    None
                },
            ),
        ...
        match self {
            ...
            FuzzPart::File(name, content) => {
                ...
                if let Some(bytes) = content {
                    ...
                    file_obj.insert(
                        "uri".to_string(),
                        json!(format!("https://example.com/{}", name)),
                    );
                }
...
impl FuzzArtifact {
    fn random(rng: &mut impl Rng) -> Self {
        ...
        Self {
            name: if rng.gen_bool(0.8) {
                Some(random_string(rng))
            } else {
                None
            },
            parts: (0..num_parts).map(|_| FuzzPart::random(rng)).collect(),
            index: if rng.gen_bool(0.8) {
...
impl FuzzJsonRpc {
    fn random(rng: &mut impl Rng) -> Self {
        ...
        Self {
            jsonrpc: if rng.gen_bool(0.9) {
                Some("2.0".to_string())
            } else {
                Some(format!("{}.0", rng.gen_range(1..=5)))
            },
            id: if rng.gen_bool(0.9) {
                if rng.gen_bool(0.5) {
...
pub fn run_fuzzer(target: &str, time_seconds: u64) {
    ...
    match target {
        ...
        _ => {
            println!(
                "Unknown target: {}. Available targets: schema, jsonrpc, message",
                target
            );
    ...
    println!(
        "   Tests per second: {}",
        tests_run / elapsed.as_secs().max(1)
    );
