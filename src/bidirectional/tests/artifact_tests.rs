...
impl TestAgentWithArtifacts {
    ...
    async fn process_message_with_file(
        &mut self,
        message_text: &str,
        file_path: &str,
        file_content: Vec<u8>,
    ...
    async fn process_message_with_data(
        &mut self,
        message_text: &str,
        ...
        for (i, artifact) in artifacts.iter().enumerate() {
            result.push_str(&format!(
                ...
                artifact
                    .name
                    .clone()
                    .unwrap_or_else(|| format!("Artifact {}", artifact.index)),
                artifact
                    .description
                    .clone()
...
async fn test_process_message_with_file() {
    ...
    let response = agent
        .process_message_with_file(message, file_path, file_content)
        .await
        .unwrap();
...
async fn test_process_message_with_data() {
    ...
    let response = agent
        .process_message_with_data(message, data_map)
        .await
        .unwrap();
