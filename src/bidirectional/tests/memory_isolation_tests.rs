...
async fn test_memory_isolation_between_sessions() -> anyhow::Result<()> {
    ...
    let task1 = create_test_task_with_session(&task1_id, TaskState::Completed, "Secret info from session 1", &session1_id);
    ...
    let task2 = create_test_task_with_session(&task2_id, TaskState::Completed, "Information from session 2", &session2_id);
    ...
    assert!(session1_tasks_output.contains(&task2_id), "Session 2 tasks should be visible");
    assert!(session1_tasks_output.contains(&task1_id), "Session 1 tasks should be visible in global memory");
...
async fn test_memory_isolation_between_users() -> anyhow::Result<()> {
    ...
    assert!(all_tasks.iter().any(|t| t.id == "user1-task"), "User 1's task should be in memory");
    assert!(all_tasks.iter().any(|t| t.id == "user2-task"), "User 2's task should be in memory");
...
async fn test_memory_isolation_with_metadata_filtering() -> anyhow::Result<()> {
    ...
    let all_tasks = agent.rolling_memory.get_all_tasks();
    let filtered_tasks: Vec<&Task> = all_tasks.iter()
        .filter(|task| {
    ...
    assert!(filtered_tasks.iter().any(|t| t.id == "user1-thread1"), "Should find user1-thread1");
    assert!(filtered_tasks.iter().any(|t| t.id == "user1-thread2"), "Should find user1-thread2");
    
    // Further filter by thread
    let thread_filtered: Vec<&Task> = filtered_tasks.into_iter()
        .filter(|task| {
            if let Some(metadata) = &task.metadata {
    ...
    assert_eq!(thread_filtered.len(), 1, "Should find 1 task for user-1, thread-1");
    assert!(thread_filtered.iter().any(|t| t.id == "user1-thread1"), "Should find user1-thread1");
...
async fn test_memory_isolation_in_task_context() -> anyhow::Result<()> {
    ...
    let project1_tasks: Vec<&Task> = all_tasks.iter()
        .filter(|task| {
            if let Some(metadata) = &task.metadata {
...
async fn test_memory_isolation_with_security_levels() -> anyhow::Result<()> {
    ...
    let all_tasks = agent.rolling_memory.get_all_tasks();
    let public_only: Vec<&Task> = all_tasks.iter()
        .filter(|task| {
            if let Some(metadata) = &task.metadata {
    ...
    assert!(public_only.iter().any(|t| t.id == "public-task"), "Should find public-task");
...
async fn test_memory_isolation_in_delegated_chain() -> anyhow::Result<()> {
    ...
    assert_eq!(agent.rolling_memory.get_all_tasks().len(), 1, "Only the original task should be in memory");
    assert!(agent.rolling_memory.get_task("original-task").is_some(), "Original task should be in memory");
    assert!(agent.rolling_memory.get_task("delegated-task1").is_none(), "Delegated task 1 should not be in memory");
    assert!(agent.rolling_memory.get_task("delegated-task2").is_none(), "Delegated task 2 should not be in memory");
...
async fn test_memory_isolation_between_organizations() -> anyhow::Result<()> {
    ...
    let all_tasks = agent.rolling_memory.get_all_tasks();
    let org1_tasks: Vec<&Task> = all_tasks.iter()
        .filter(|task| {
            if let Some(metadata) = &task.metadata {
    ...
    assert!(org1_tasks.iter().any(|t| t.id == "org1-task"), "Should find org1-task");
...
async fn test_memory_isolation_across_agent_restarts() -> anyhow::Result<()> {
    ...
    assert_eq!(agent1.rolling_memory.get_all_tasks().len(), 1, "Task should be in memory");
    ...
    assert_eq!(agent2.rolling_memory.get_all_tasks().len(), 0, "Memory should be empty after restart");
...
async fn test_memory_isolation_with_permissions() -> anyhow::Result<()> {
    ...
    let all_tasks = agent.rolling_memory.get_all_tasks();
    let standard_accessible: Vec<&Task> = all_tasks.iter()
        .filter(|task| {
            if let Some(metadata) = &task.metadata {
    ...
    assert_eq!(standard_accessible.len(), 1, "Should find only standard tasks");
    assert!(standard_accessible.iter().any(|t| t.id == "standard-task"), "Should find standard-task");
...
async fn test_memory_isolation_for_similar_tasks() -> anyhow::Result<()> {
    ...
    // Filter by user ID
    let all_tasks = agent.rolling_memory.get_all_tasks();
    let user1_tasks: Vec<&Task> = all_tasks.iter()
        .filter(|task| {
            if let Some(metadata) = &task.metadata {
                if let Some(user_id) = metadata.get("user_id") {
                    if let Some(user_id_str) = user_id.as_str() {
    ...
    assert!(user1_tasks.iter().any(|t| t.id == "user1-search"), "Should find user1-search");
    
    Ok(())
}

// Helper function to create test tasks with a specific session ID
fn create_test_task_with_session(id: &str, state: TaskState, message_text: &str, session_id: &str) -> Task {
    Task {
        id: id.to_string(),
        status: TaskStatus {
            state,
