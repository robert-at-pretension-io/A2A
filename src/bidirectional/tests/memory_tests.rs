...
async fn test_rolling_memory() {
    ...
    assert_eq!(
        tasks_chronological.len(),
        2,
        "Should have 2 tasks in chronological order"
    );
    assert_eq!(
        tasks_chronological[0].id, "task-1",
        "First task should be task-1"
    );
    assert_eq!(
        tasks_chronological[1].id, "task-2",
    ...
    assert_eq!(
        all_tasks.len(),
        2,
        "Should have only 2 tasks after adding 3 (size limit)"
    );
    assert!(
        size_limited_memory.get_task("task-1").is_none(),
        "Oldest task should be pruned"
    );
    assert!(
        size_limited_memory.get_task("task-2").is_some(),
        "Middle task should still exist"
    );
    assert!(
        size_limited_memory.get_task("task-3").is_some(),
...
async fn test_rolling_memory_outgoing_only() -> anyhow::Result<()> {
    ...
    let incoming_task = create_test_task_with_metadata(
        "incoming-1",
        TaskState::Completed,
    ...
    assert!(
        not_stored_task.is_none(),
        "Incoming task should NOT be in rolling memory"
    );

    // Test multiple outgoing tasks
    let outgoing_task2 =
        create_test_task("outgoing-2", TaskState::Working, "Another outgoing task");
...
async fn test_agent_implementation_outgoing_only() -> anyhow::Result<()> {
    ...
    assert!(
        agent.rolling_memory.get_task(&incoming_id).is_none(),
        "Incoming task should not be in rolling memory"
    );
...
async fn test_delegated_tasks_not_in_memory() -> anyhow::Result<()> {
    ...
    assert!(
        agent.rolling_memory.get_task("outgoing-1").is_some(),
        "Outgoing task should be in rolling memory"
    );
    assert!(
        agent.rolling_memory.get_task("delegated-1").is_none(),
...
async fn test_repl_memory_commands() -> anyhow::Result<()> {
    ...
    assert!(
        memory_output.contains("Rolling Memory"),
        "Memory output should contain header"
    );
    assert!(
        memory_output.contains("task-1"),
        "Memory output should contain task-1"
    );
    assert!(
        memory_output.contains("task-2"),
        "Memory output should contain task-2"
    );

    // Test :memoryTask command
    let memory_task_output = commands::handle_memory_task(&agent, "task-1")?;
    assert!(
        memory_task_output.contains("Memory Task Details"),
        "Memory task output should contain header"
    );
    assert!(
        memory_task_output.contains("Task 1 test"),
        "Memory task output should contain task content"
    );
