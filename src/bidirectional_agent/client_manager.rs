...
pub struct ClientManager {
    /// Cache of clients, keyed by agent ID (typically agent name from card).
    // Use A2aClient directly as it's Clone
    pub(crate) clients: Arc<DashMap<String, A2aClient>>, // Make pub(crate) for tests
    /// Reference to the agent registry to get agent URLs and auth info.
    registry: Arc<AgentRegistry>,
    /// Agent's own configuration (needed for client TLS/auth setup).
    self_config: Arc<BidirectionalAgentConfig>,
...
impl ClientManager {
    ...
    pub async fn get_or_create_client(&self, agent_id: &str) -> Result<A2aClient, ClientError> {
        ...
        // 3. Create the A2aClient instance using the custom http_client
        let mut a2a_client = A2aClient::new_with_client(agent_url, http_client);
    ...
    fn build_http_client(&self) -> Result<reqwest::Client> {
            ...
            (&self.self_config.auth.client_cert_path, &self.self_config.auth.client_key_path)
        {
            ...
            // Combine cert and key bytes for identity
            let mut combined_pem = cert_bytes;
            ...
            let identity = reqwest::Identity::from_pem(&combined_pem)
                 .with_context(|| format!("Failed to create identity from cert '{}' and key '{}'", cert_path, key_path))?;
    ...
    pub async fn send_task(&self, agent_id: &str, params: TaskSendParams) -> Result<Task, ClientError> {
        ...
        // Extract text from message parts - assuming first part is TextPart
        let text = match params.message.parts.first() {
            Some(crate::types::Part::TextPart(text_part)) => &text_part.text,
            _ => "Default message text" // Fallback text if not a TextPart
        };
    ...
    /// Periodically polls the status of delegated tasks.
    /// This should be run in a background task managed by the main agent loop.
    #[cfg(feature = "bidir-delegate")] // Only compile if delegation feature is enabled
    pub async fn run_delegated_task_poll_loop(&self, interval: chrono::Duration) {
        println!("🕰️ Starting delegated task polling loop (interval: {:?})", interval);
        let std_interval = match interval.to_std() {
             Ok(d) => d,
             Err(_) => {
                 eprintln!("Error converting poll interval duration, defaulting to 30 seconds.");
        ...
        loop {
            ...
            // --- Find Delegated Tasks ---
            // This requires access to the TaskRepository and the TaskOrigin side-table.
            // Actual implementation would use the TaskRepositoryExt trait methods.
            // Placeholder:
            let delegated_tasks: Vec<(String, String, String)> = Vec::new();
            println!("  (Placeholder: Found {} active delegated tasks)", delegated_tasks.len());
            ...
            for (local_task_id, agent_id, remote_task_id) in delegated_tasks {
                ...
                match self.get_task_status(&agent_id, &remote_task_id).await {
                    Ok(remote_task) => {
                        println!("    Remote status: {:?}", remote_task.status.state);
                        // Update local task status based on remote status
                        // Requires access to TaskRepositoryExt
                        // Example: self.task_repository.update_delegated_task_status(&local_task_id, &remote_task).await;

                        if matches!(remote_task.status.state, crate::types::TaskState::Completed | crate::types::TaskState::Failed | crate::types::TaskState::Canceled) {
                             println!("    ✅ Remote task '{}' reached final state.", remote_task_id);
                             // Trigger result synthesis or finalization logic here.
                        }
                    }
                    Err(e) => {
                         println!("    ⚠️ Failed to get status for remote task '{}': {}", remote_task_id, e);
                         // Handle error (e.g., mark local task as failed)
                         // Example: self.task_repository.mark_task_failed(&local_task_id, &e.to_string()).await;
                    }
                }
            }
...
mod tests {
    ...
    async fn test_get_or_create_client_with_matching_auth() {
        ...
        // Check if auth header is configured (internal check)
        assert_eq!(client.auth_header.as_deref(), Some("Authorization"));
        assert_eq!(client.auth_value.as_deref(), Some("Bearer my-secret-token"));
        assert!(manager.clients.contains_key(agent_name));
    }
    ...
    async fn test_get_or_create_client_no_matching_auth() {
        ...
        assert!(client_result.is_ok());
        let client = client_result.unwrap();
        assert!(client.auth_header.is_none()); // No auth should be configured
    }
    ...
    async fn test_get_client_for_unknown_agent() {
        ...
        assert!(client_result.is_err());
        assert!(matches!(client_result.unwrap_err(), ClientError::Other(_)));
        assert!(manager.clients.is_empty()); // No client should be cached
    }
}
