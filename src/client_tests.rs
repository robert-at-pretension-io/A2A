...
use crate::client::errors::{error_codes, A2aError, ClientError};
...
mod error_handling_tests {
    use super::*;
    use crate::client::A2aClient;
    use tokio::time; // Add A2aClient import
    ...
    async fn test_task_not_found_error() -> Result<(), Box<dyn Error>> {
        ...
        let result = client
            .get_task_with_error_handling(&non_existent_task_id)
            .await;
        ...
        match result {
            Err(ClientError::A2aError(a2a_error)) => {
                println!(
                    "Error code: {}, message: {}",
                    a2a_error.code, a2a_error.message
                );
                assert_eq!(
                    a2a_error.code,
                ...
                assert!(
                    a2a_error.message.contains("Task not found"),
                    "Error message should mention task not found"
                );
                assert!(
                    a2a_error.is_task_not_found(),
    ...
    async fn test_invalid_parameters_error() -> Result<(), Box<dyn Error>> {
        ...
        match result {
            Err(ClientError::A2aError(a2a_error)) => {
                ...
                assert!(
                    a2a_error.message.contains("Invalid parameters"),
                    "Error message should mention invalid parameters"
                );
                assert!(
                    a2a_error.is_invalid_params(),
    ...
    async fn test_method_not_found_error() -> Result<(), Box<dyn Error>> {
        ...
        match result {
            Err(ClientError::A2aError(a2a_error)) => {
                ...
                assert!(
                    a2a_error.message.contains("Method not found"),
                    "Error message should mention method not found"
                );
                assert!(
                    a2a_error.is_method_not_found(),
    ...
    async fn test_task_not_cancelable_error() -> Result<(), Box<dyn Error>> {
        ...
        assert!(
            result.is_err(),
            "Expected error for canceling completed task"
        );
        ...
        match result {
            Err(ClientError::A2aError(a2a_error)) => {
                ...
                assert!(
                    a2a_error.message.contains("Task cannot be canceled"),
                    "Error message should mention task cancellation"
                );
                assert!(
                    a2a_error.is_task_not_cancelable(),
    ...
    async fn test_authentication_error() -> Result<(), Box<dyn Error>> {
        ...
        let client_error = ClientError::ReqwestError {
            msg: "401 Unauthorized: Authentication required".to_string(),
