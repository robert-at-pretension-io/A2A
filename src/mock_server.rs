use crate::client::errors::{error_codes, A2aError};
use crate::types::{
    AgentAuthentication, AgentCapabilities, AgentCard, AgentSkill, Artifact, AuthenticationInfo,
    DataPart, FileContent, FilePart, Message, Part, PushNotificationConfig, Role,
    TaskPushNotificationConfig, TaskState, TaskStatus, TextPart,
};
...
fn create_agent_card_with_auth(require_auth: bool) -> AgentCard {
    ...
    let authentication = if require_auth {
        ...
        let schemes = if std::thread::current()
            .name()
            .unwrap_or("")
            .contains("auth_test")
        {
...
// Removed unused batch_storage and file_storage parameters
async fn handle_a2a_request_with_auth(
    task_storage: TaskStorage,
    ...
    println!(
        "DEBUG: Thread name: {}, URL: {}, Is auth test: {}",
        thread_name, req_url, is_auth_test
    );
        ...
        if req.uri().path() == "/.well-known/agent.json" {
            ...
            if !auth_valid {
                ...
                if let Some(key) = api_key {
                    println!(
                        "AUTH TEST: Found X-API-Key header: {}",
    ...
    let message_opt =
        if request.get("method").and_then(|m| m.as_str()).unwrap_or("") == "tasks/send" {
            request
                .get("params")
        ...
        match method {
            ...
            "tasks/send" => {
                ...
                let task_id = params
                    ...
                    .unwrap_or_else(|| {
                        format!("mock-task-{}", chrono::Utc::now().timestamp_millis())
                    });
                ...
                let session_id = params
                    ...
                    .map(|s| s.to_string())
                    .unwrap_or_else(|| {
                        format!("mock-session-{}", chrono::Utc::now().timestamp_millis())
                    });
                ...
                if is_followup {
                    ...
                    // Also immediately transition to Working state for better test behavior
                    {
                        ...
                        if let Some(existing_task) = storage.get_mut(&task_id) {
                            ...
                            println!(
                                "📝 Task {} transitioned to Working state after receiving input",
                                task_id
                            );
                        }
                    }
                ...
                let task = match &simulation_config {
                ...
                if let Some((duration_ms, require_input, should_fail, fail_message)) =
                    &simulation_config
                {
                                    ...
                                    .is_some())
                        {
                            ...
                            return Ok(Response::new(Body::from(
                                create_error_response(
                                    ...
                                    &format!(
                                        "Task cannot be canceled: {} is in {} state",
            ...
            "tasks/sendSubscribe" => {
                ...
                let (text_chunks, artifact_types, chunk_delay_ms, final_state) = streaming_config
                    .unwrap_or((
                        5,
                        vec!["text".to_string(), "data".to_string(), "file".to_string()],
                        300,
                        "completed".to_string(),
                    ));
                ...
                let session_id =
                    format!("stream-session-{}", chrono::Utc::now().timestamp_millis());
                ...
                tokio::spawn(async move {
                    ...
                    if task_types.contains(&"text".to_string()) {
                        ...
                        for i in 0..num_text_chunks {
                            ...
                            let (artifact, artifact_update, _) =
                                generate_mock_text_artifact(&task_id, i as usize, is_last, append);
                    ...
                    if task_types.contains(&"data".to_string()) {
                        ...
                        let (artifact, artifact_update, _) =
                            generate_mock_data_artifact(&task_id, artifact_count as usize, true);
                    ...
                    if task_types.contains(&"file".to_string()) {
                        ...
                        let (artifact, artifact_update, _) =
                            generate_mock_file_artifact(&task_id, artifact_count as usize, true);
                    ...
                    if task_types.contains(&"text".to_string()) {
                        ...
                        for i in 0..num_text_chunks {
                            ...
                            let text_part = TextPart {
                                type_: "text".to_string(),
                                text: format!(
                                    "Continued chunk {} after resubscribe for task {}",
                                    i, task_id
                                ),
                                metadata: None,
                            };
            ...
            "tasks/pushNotification/set" => {
                ...
                println!(
                    "Setting push notification webhook for task {}: {}",
                    task_id, webhook_url
                );
...
/// }
/// ```
fn parse_task_simulation_config(
    params: &Option<&Value>,
) -> Option<(u64, bool, bool, Option<String>)> {
    if let Some(params_value) = params {
        if let Some(metadata) = params_value.get("metadata").and_then(|m| m.as_object()) {
            ...
            let require_input = metadata
                .get("_mock_require_input")
                .and_then(|r| r.as_bool())
                .unwrap_or(false);

            // Extract failure simulation
            let should_fail = metadata
                .get("_mock_fail")
                .and_then(|f| f.as_bool())
                .unwrap_or(false);

            // Extract custom failure message
            let fail_message = metadata
                .get("_mock_fail_message")
                .and_then(|m| m.as_str())
                .map(|s| s.to_string());
...
fn parse_streaming_config(params: &Option<&Value>) -> Option<(u64, Vec<String>, u64, String)> {
    if let Some(params_value) = params {
        if let Some(metadata) = params_value.get("metadata").and_then(|m| m.as_object()) {
            ...
            let text_chunks = metadata
                .get("_mock_stream_text_chunks")
                .and_then(|c| c.as_u64())
                .unwrap_or(5);

            // Extract artifact_types parameter (default to all types if not provided)
            let artifact_types = metadata
                .get("_mock_stream_artifact_types")
            ...
            let chunk_delay_ms = metadata
                .get("_mock_stream_chunk_delay_ms")
                .and_then(|d| d.as_u64())
                .unwrap_or(300);

            // Extract final_state parameter (default to "completed" if not provided)
            let final_state = metadata
                .get("_mock_stream_final_state")
                .and_then(|s| s.as_str())
                .unwrap_or("completed")
                .to_string();
...
fn generate_mock_text_artifact(
    task_id: &str,
    index: usize,
    is_last: bool,
    append: bool,
...
fn generate_mock_data_artifact(
    task_id: &str,
    index: usize,
    is_last: bool,
) -> (Artifact, Value, u64) {
...
fn generate_mock_file_artifact(
    task_id: &str,
    index: usize,
    is_last: bool,
) -> (Artifact, Value, u64) {
...
async fn simulate_task_lifecycle(
    ...
    println!(
        "   Duration: {}ms, Require input: {}, Should fail: {}",
    ...
    if require_input {
        ...
        loop {
            ...
            if input_received {
                println!(
                    "📨 Input received for task {}, continuing processing",
                    task_id
                );
                ...
                // Update status to Working again
                {
                    ...
                    if let Some(task) = storage.get_mut(&task_id) {
                        ...
                        if task.current_status.state == TaskState::InputRequired {
                            ...
                            println!(
                                "ℹ️ Task {} already updated, current state: {:?}",
                                task_id, task.current_status.state
                            );
                        }
                    }
    ...
    // Final state transition (Completed or Failed)
    {
        ...
        if let Some(task) = storage.get_mut(&task_id) {
            ...
            if should_fail {
                ...
                let fail_text =
                    fail_message.unwrap_or_else(|| "Task failed to complete.".to_string());
...
fn create_response_artifacts(message_opt: &Option<Value>) -> Vec<Artifact> {
    ...
    if let Some(message) = message_opt {
        ...
        if let Some(parts) = message.get("parts").and_then(|p| p.as_array()) {
            ...
            let has_data = parts
                .iter()
                .any(|p| p.get("type").and_then(|t| t.as_str()) == Some("data"));
