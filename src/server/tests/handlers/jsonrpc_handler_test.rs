...
use crate::types::{
    Message, Part, PushNotificationConfig, Role, Task, TaskIdParams, TaskPushNotificationConfig,
    TaskQueryParams, TaskSendParams, TaskState, TaskStatus, TextPart,
};
...
async fn test_tasks_send_long_text() {
    ...
    if json["error"].is_object() {
        ...
        assert!(
            ...
            "Expected Invalid Params or Internal Error for too long text, got {}",
            code
        );
    } else {
...
async fn test_tasks_send_invalid_utf8() {
    ...
    assert!(
        error_code == -32700 || error_code == -32602,
        "Expected either Parse error (-32700) or Invalid Params (-32602), got {}",
        error_code
    );
}
...
async fn test_tasks_send_follow_up_to_canceled() {
    ...
    assert!(json["error"]["message"]
        .as_str()
        .unwrap()
        .contains("cannot accept follow-up"));
}
...
async fn test_push_notification_set_overwrite() {
    ...
    notification_service
        .set_push_notification(first_push_config)
        .await
        .unwrap();
    ...
    notification_service
        .set_push_notification(second_push_config)
        .await
        .unwrap();
...
async fn test_push_notification_set_empty_auth_schemes() {
    ...
    notification_service
        .set_push_notification(push_config)
        .await
        .unwrap();

    // Verify stored config
    let stored_config = repository
        .get_push_notification_config(&task_id)
        .await
        .unwrap()
        .unwrap();
...
async fn test_tasks_pushnotification_set_endpoint() {
    ...
    notification_service
        .set_push_notification(task_push_config)
        .await
        .unwrap();
...
async fn test_incorrect_content_type() {
    ...
    let body =
        r#"{"jsonrpc": "2.0", "method": "tasks/get", "params": {"id": "task-123"}, "id": "1"}"#;
...
async fn test_wild_concurrent_cancel_followup() {
    ...
    let res_create = jsonrpc_handler(
        req_create,
        task_service.clone(),
        streaming_service.clone(),
        notification_service.clone(),
    ...
    let cancel_handle = tokio::spawn(async move {
        ...
        jsonrpc_handler(
            req_cancel,
            task_service_clone1,
            streaming_service_clone1,
            notification_service_clone1,
        )
        .await
    });
    ...
    let followup_handle = tokio::spawn(async move {
        ...
        jsonrpc_handler(
            req_followup,
            task_service_clone2,
            streaming_service_clone2,
            notification_service_clone2,
        )
        .await
    });
    ...
    assert!(
        cancel_succeeded ^ followup_succeeded,
        "Exactly one operation (cancel or follow-up) should succeed in the race"
    );
    ...
    if cancel_succeeded {
        assert_eq!(
            ...
            TaskState::Canceled,
            "Final state should be Canceled if cancel won"
        );
        assert!(
            json_followup["error"].is_object(),
            "Follow-up should have failed if cancel won"
        );
        ...
        assert!(
            err_code == -32602 || err_code == -32002,
            "Follow-up error code mismatch"
        );
    } else {
        // Follow-up succeeded
        assert_eq!(
            final_task.status.state,
            TaskState::Completed,
            "Final state should be Completed if follow-up won"
        );
        assert!(
            json_cancel["error"].is_object(),
            "Cancel should have failed if follow-up won"
        );
        assert_eq!(
            json_cancel["error"]["code"], -32002,
            "Cancel error code should be TaskNotCancelable"
        ); // Task completed
    }
}
...
impl TaskRepository for FailOnSecondSaveRepo {
    ...
    async fn save_task(&self, task: &Task) -> Result<(), ServerError> {
        if task.id == self.task_id_to_fail {
            ...
            *count += 1;
            if *count == 2 {
                // Fail on the second save call for the specific task
                println!("Simulating failure on second save for task {}", task.id);
                return Err(ServerError::Internal(
                    "Simulated repo failure on second save".to_string(),
                ));
            }
        }
...
async fn test_wild_repo_failure_mid_process() {
    ...
    assert_eq!(
        ...
        "Error code should be Internal Server Error"
    );
    assert!(
        json["error"]["message"]
            .as_str()
            .unwrap()
            .contains("Simulated repo failure"),
    ...
    assert!(
        task_in_repo.is_some(),
        "Task should exist in repo (from first save)"
    );
    ...
    assert!(
        initial_state == TaskState::Working || initial_state == TaskState::Submitted,
        "Task state in repo should be the initial state (Working or Submitted), but was {:?}",
        initial_state
    );
    ...
    assert_eq!(
        history.len(),
        1,
        "Only one history entry (initial state) should exist"
    );
    assert_eq!(
        history[0].status.state, initial_state,
...
async fn test_wild_stream_cancel_during_setup() {
    ...
    if cancel_succeeded {
        assert_eq!(
            json_cancel["result"]["status"]["state"], "canceled",
            "If cancel succeeded, result state should be canceled"
        );
    }
    ...
    assert!(
        is_sse_response(&res_stream),
        "Stream request should return SSE"
    );
    ...
    if cancel_got_not_cancelable {
        ...
        assert_eq!(
            final_task.status.state,
            TaskState::Completed,
            "Final state should be Completed if cancel got -32002"
        );
    } else {
        ...
        assert_eq!(
            final_task.status.state,
            TaskState::Canceled,
            "Final state should be Canceled if cancel succeeded or got -32001"
        );
    }
...
async fn test_wild_push_notification_complex_extra() {
    ...
    assert!(
        set_result.is_ok(),
        "Setting complex push notification failed: {:?}",
        set_result.err()
    );
    ...
    let retrieved_config = notification_service
        .get_push_notification(get_params)
        .await
        .unwrap();
    ...
    assert_eq!(
        retrieved_extra_val, complex_extra,
        "Complex 'extra' metadata does not match"
    );
}
...
async fn test_wild_conflicting_mock_metadata() {
    ...
    assert_eq!(
        ...
        "Task should complete as message metadata overrides task metadata"
    );
...
async fn test_wild_resubscribe_immediate_cancel() {
    ...
    metadata_map.insert(
        "_mock_remain_working".to_string(),
    ...
    let resub_handle = tokio::spawn(async move {
        ...
        streaming_service_clone1
            .resubscribe_to_task(json!("resub-req"), task_id_clone1)
            .await
    });
    ...
    let cancel_handle = tokio::spawn(async move {
        ...
        task_service_clone2
            .cancel_task(serde_json::from_value(params_cancel).unwrap())
            .await
    });
    ...
    assert!(
        res_resub.unwrap().is_ok(),
        "Resubscribe should return a stream"
    );
...
async fn test_wild_get_history_length_exceeds() {
    ...
    println!(
        "Note: History population in tasks/get result is not fully implemented in TaskService."
    );
}
...
async fn test_wild_simultaneous_create_same_id() {
    ...
    assert!(
        success1 ^ success2,
        "Exactly one create request should succeed"
    );
    ...
    if success1 {
        assert!(
            json2["error"].is_object(),
            "Second request should have failed"
        );
        assert_eq!(
            json2["error"]["code"], -32602,
            "Error code for second request should be Invalid Params (-32602)"
        );
        // Removed brittle check for specific error message content
    } else {
        assert!(
            json1["error"].is_object(),
            "First request should have failed"
        );
        assert_eq!(
            json1["error"]["code"], -32602,
            "Error code for first request should be Invalid Params (-32602)"
        );
        // Removed brittle check for specific error message content
    }
...
async fn test_wild_malformed_jsonrpc_structure() {
    ...
    assert_eq!(
        json_v1["error"]["code"], -32001,
        "Expected TaskNotFound for v1.0 request"
    );
    ...
    let req_extra = Request::builder()
        ...
        .uri("/")
        .header("Content-Type", "application/json")
        .body(Body::from(serde_json::to_string(&body_extra).unwrap()))
        .unwrap();
    let res_extra = jsonrpc_handler(
        req_extra,
        task_service.clone(),
        streaming_service.clone(),
        notification_service.clone(),
    ...
    assert_eq!(
        json_extra["error"]["code"], -32001,
        "Expected TaskNotFound for request with extra field"
    );
    ...
    let req_missing_rpc = Request::builder()
        ...
        .header("Content-Type", "application/json")
        .body(Body::from(
            serde_json::to_string(&body_missing_rpc).unwrap(),
        ))
        .unwrap();
    let res_missing_rpc = jsonrpc_handler(
        req_missing_rpc,
        task_service.clone(),
        streaming_service.clone(),
        notification_service.clone(),
    ...
    assert_eq!(
        json_missing_rpc["error"]["code"], -32001,
        "Expected TaskNotFound for request missing jsonrpc field"
    );
}
...
async fn test_wild_followup_wrong_role() {
    ...
    assert!(
        ...
        "Follow-up with wrong role unexpectedly failed"
    );
    assert_eq!(json["result"]["id"], task_id);
    assert_eq!(
        json["result"]["status"]["state"], "completed",
        "Task should complete even with wrong role follow-up (current logic)"
    );
