...
async fn test_create_streaming_task_returns_stream() {
    ...
    while let Some(message) = stream.next().await {
        ...
        assert!(
            message.starts_with("data: "),
            "Message should start with 'data: '"
        );
        assert!(
            message.ends_with("\n\n"),
    ...
    assert!(
        final_value.is_boolean() || final_value.is_null(),
        "Final value should be boolean or null, got: {:?}",
        final_value
    );
}
...
async fn test_resubscribe_to_active_task() {
    ...
    let stream = service
        .resubscribe_to_task(request_id, task_id.clone())
        .await
        .unwrap();
    ...
    let state_str = messages[0]["result"]["status"]["state"]
        .as_str()
        .unwrap_or("");
    assert!(
        state_str.eq_ignore_ascii_case("working"),
        "State should be Working (case insensitive)"
    );

    // Last message should have final=true
    assert_eq!(
        messages[1]["result"]["final"], true,
        "Last message should have final=true"
    );
}
...
async fn test_resubscribe_to_completed_task() {
    ...
    assert_eq!(
        messages[1]["result"]["final"], true,
        "Second message should have final=true"
    );
    // Allow for either "completed" or "Completed" - case doesn't matter in this context
    let state_str = messages[1]["result"]["status"]["state"]
        .as_str()
        .unwrap_or("");
    assert!(
        state_str.eq_ignore_ascii_case("completed"),
        "State should be Completed (case insensitive)"
    );
}
...
async fn test_resubscribe_to_nonexistent_task() {
    ...
    let result = service
        .resubscribe_to_task(request_id, non_existent_id.clone())
        .await;

    // Assert
    assert!(
        result.is_err(),
        "Resubscribing to non-existent task should fail"
    );
    ...
    if let Err(err) = result {
        match err {
            ServerError::TaskNotFound(msg) => {
                ...
                assert!(
                    msg.contains(&non_existent_id),
                    "Error message '{}' should contain the task ID '{}'",
                    msg,
                    non_existent_id
                );
            }
