use crate::types::{AgentCard, Message, Part, Role, Task, TaskSendParams, TextPart};
...
pub mod errors; // Add error handling module // Add specialized error handling module with new functions

use errors::{A2aError, ClientError};
...
impl A2aClient {
    ...
    pub async fn send_jsonrpc<T: serde::de::DeserializeOwned>(
        ...
        let mut http_request = self.http_client.post(&self.base_url).json(&request);
    ...
    pub async fn get_agent_card(&self) -> Result<AgentCard, ClientError> {
        ...
        if response.status() != StatusCode::OK {
            return Err(ClientError::ReqwestError {
                msg: format!("Failed to get agent card: {}", response.status()),
                status_code: Some(response.status().as_u16()),
            });
        }
    ...
    pub async fn send_task(
        ...
        let params_value = match serde_json::to_value(params) {
            Ok(v) => v,
            Err(e) => {
                return Err(ClientError::JsonError(format!(
    ...
    pub async fn simulate_task_lifecycle(
        ...
        metadata.insert(
            "_mock_duration_ms".to_string(),
            serde_json::Value::Number(serde_json::Number::from(duration_ms)),
        );
        metadata.insert(
            "_mock_require_input".to_string(),
            serde_json::Value::Bool(require_input),
        );
        metadata.insert(
            "_mock_fail".to_string(),
            serde_json::Value::Bool(should_fail),
        );

        if let Some(message) = fail_message {
            metadata.insert(
                "_mock_fail_message".to_string(),
    ...
    pub async fn get_skill_details_with_error_handling(
        &mut self,
        skill_id: &str,
    ...
    pub async fn get_batch_with_error_handling(
        &mut self,
        batch_id: &str,
    ) -> Result<String, ClientError> {
        // For now, just use the tasks/get endpoint with a non-existent method
    ...
    pub async fn download_file_with_error_handling(
        &mut self,
        file_id: &str,
    ) -> Result<String, ClientError> {
        // For now, just use the tasks/get endpoint with a non-existent method
