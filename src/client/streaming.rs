...
pub type StreamingResponseStream =
    Pin<Box<dyn Stream<Item = Result<StreamingResponse, ClientError>> + Send>>;

impl A2aClient {
    /// Send a task with streaming response enabled via SSE (typed error version)
    pub async fn send_task_subscribe_typed(
        &mut self,
        text: &str,
    ) -> Result<StreamingResponseStream, ClientError> {
        // Call with empty metadata
        self.send_task_subscribe_with_metadata_typed(text, &json!({}))
            .await
    }
    ...
    pub async fn send_task_subscribe(
        &mut self,
        text: &str,
    ) -> Result<StreamingResponseStream, Box<dyn Error>> {
    ...
    /// ).await?;
    /// ```
    pub async fn send_task_subscribe_with_metadata_str(
        &mut self,
        text: &str,
        ...
        self.send_task_subscribe_with_metadata(text, &metadata)
            .await
    }
    ...
    /// ).await?;
    /// ```
    pub async fn send_task_subscribe_with_metadata_typed(
        &mut self,
        text: &str,
        ...
        self.send_streaming_request_typed("tasks/sendSubscribe", serde_json::to_value(params)?)
            .await
    }

    /// Send a task with streaming response enabled via SSE and metadata (backward compatible)
    pub async fn send_task_subscribe_with_metadata(
        &mut self,
        text: &str,
        metadata: &serde_json::Value,
    ) -> Result<StreamingResponseStream, Box<dyn Error>> {
        match self
            .send_task_subscribe_with_metadata_typed(text, metadata)
            .await
        {
            Ok(val) => Ok(val),
            Err(err) => Err(Box::new(err)),
    ...
    pub async fn resubscribe_task_typed(
        &mut self,
        task_id: &str,
    ) -> Result<StreamingResponseStream, ClientError> {
        // Call with no metadata
        self.resubscribe_task_with_metadata_typed(task_id, &json!({}))
            .await
    }
    ...
    /// * `metadata` - JSON value containing metadata
    pub async fn resubscribe_task_with_metadata_typed(
        &mut self,
        task_id: &str,
        metadata: &serde_json::Value,
    ) -> Result<StreamingResponseStream, ClientError> {
        ...
        // Build the SSE request
        self.send_streaming_request_typed("tasks/resubscribe", serde_json::to_value(params)?)
            .await
    }

    /// Resubscribe to a task's streaming updates with metadata (backward compatible)
    pub async fn resubscribe_task_with_metadata(
        &mut self,
        task_id: &str,
        metadata: &serde_json::Value,
    ) -> Result<StreamingResponseStream, Box<dyn Error>> {
        match self
            .resubscribe_task_with_metadata_typed(task_id, metadata)
            .await
        {
            Ok(val) => Ok(val),
            Err(err) => Err(Box::new(err)),
    ...
    pub async fn send_streaming_request_typed(
        &mut self,
        method: &str,
        params: Value,
    ) -> Result<StreamingResponseStream, ClientError> {
        ...
        if !response.status().is_success() {
            return Err(ClientError::ReqwestError {
                msg: format!("Request failed with status: {}", response.status()),
                    ...
                    match serde_json::from_str::<Value>(&event.data) {
                        Ok(json_data) => {
                            ...
                            if let Some(error) = json_data.get("error") {
                                ...
                                return Err(ClientError::A2aError(A2aError::new(
                                    code, message, data,
                                )));
                            }
                            ...
                            if let Some(result) = json_data.get("result") {
                                ...
                                if let Some(artifact) = result.get("artifact") {
                                    match serde_json::from_value::<Artifact>(artifact.clone()) {
                                        Ok(artifact_obj) => {
                                            Ok(StreamingResponse::Artifact(artifact_obj))
                                        }
                                        Err(e) => Err(ClientError::JsonError(format!(
                                ...
                                else if let Some(is_final) =
                                    result.get("final").and_then(|f| f.as_bool())
                                {
                                    if is_final {
                                        match serde_json::from_value::<Task>(result.clone()) {
                                            Ok(task) => Ok(StreamingResponse::Final(task)),
                                            Err(e) => Err(ClientError::JsonError(format!(
                                                "Failed to parse final task: {}",
                                                e
                                            ))),
                                        }
                                    } else {
                                ...
                                Err(ClientError::Other(
                                    "Invalid JSON-RPC response: missing 'result' field".to_string(),
                                ))
                            }
                        }
                        Err(e) => Err(ClientError::JsonError(format!(
    ...
    pub fn create_text_message(&self, text: &str) -> Message {
        use crate::types::{Part, Role, TextPart};
...
mod tests {
    ...
    async fn test_send_task_subscribe() {
        ...
        let mut stream = client
            .send_task_subscribe_typed("Test streaming request")
            .await
            .unwrap();
