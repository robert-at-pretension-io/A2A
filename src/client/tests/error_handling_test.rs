...
async fn test_authentication_error() -> Result<(), Box<dyn Error>> {
    ...
    // This should succeed
    let task = auth_client
        .send_task("This is an authenticated request", None)
...
async fn test_skill_not_found_error() -> Result<(), Box<dyn Error>> {
    ...
    let result = client
        .get_skill_details_with_error_handling(&non_existent_skill_id)
        .await;
...
async fn test_batch_not_found_error() -> Result<(), Box<dyn Error>> {
    ...
    let result = client
        .get_batch_with_error_handling(&non_existent_batch_id)
        .await;
...
async fn test_file_not_found_error() -> Result<(), Box<dyn Error>> {
    ...
    let result = client
        .download_file_with_error_handling(&non_existent_file_id)
        .await;
