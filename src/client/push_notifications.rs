...
use std::error::Error; // Removed json

use crate::client::errors::ClientError;
use crate::client::A2aClient;
// use crate::client::error_handling::ErrorCompatibility; // Unused
use crate::types::{
    AuthenticationInfo, PushNotificationConfig, /* TaskPushNotificationConfig, */ TaskIdParams,
}; // Removed unused
...
impl A2aClient {
    ...
    pub async fn set_task_push_notification_typed(
        ...
        let response: Value = self
            .send_jsonrpc("tasks/pushNotification/set", serde_json::to_value(params)?)
            .await?;
        ...
        Err(ClientError::Other(
            "Invalid response format for push notification".to_string(),
        ))
    }
    ...
    pub async fn set_task_push_notification(
        ...
        match self
            .set_task_push_notification_typed(task_id, webhook_url, auth_scheme, token)
            .await
        {
            Ok(val) => Ok(val),
    ...
    pub async fn get_task_push_notification_typed(
        &mut self,
        task_id: &str,
    ) -> Result<PushNotificationConfig, ClientError> {
        // Changed return type
        ...
        // Send request and return result
        let response: Value = self
            .send_jsonrpc("tasks/pushNotification/get", serde_json::to_value(params)?)
            .await?;
        ...
        if let Some(config) = response.get("pushNotificationConfig") {
            if let Ok(config_parsed) =
                serde_json::from_value::<PushNotificationConfig>(config.clone())
            {
        ...
        Err(ClientError::Other(
            "Invalid response format for push notification config".to_string(),
        ))
    }
...
mod tests {
    ...
    async fn test_set_task_push_notification() {
        ...
        let result = client
            .set_task_push_notification_typed(task_id, webhook_url, None, None)
            .await
            .unwrap();
    ...
    async fn test_set_task_push_notification_with_auth() {
        ...
        // Call the _typed version
        let result = client
            .set_task_push_notification_typed(task_id, webhook_url, Some(auth_scheme), Some(token))
            .await
            .unwrap();
    ...
    async fn test_get_task_push_notification() {
        ...
        // Call the _typed version
        let config = client
            .get_task_push_notification_typed(task_id)
            .await
            .unwrap();
    ...
    async fn test_get_task_push_notification_with_auth() {
        ...
        assert_eq!(config.url, webhook_url);
        assert_eq!(
            config.authentication.as_ref().unwrap().schemes[0],
            auth_scheme
        );
