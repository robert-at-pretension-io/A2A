...
use tokio::sync::Mutex; // Use Tokio's Mutex for the client // Add StreamExt for .next()
...
pub async fn run_integration_tests(config: TestRunnerConfig) -> Result<(), Box<dyn Error>> {
    ...
    println!(
        "{}",
        "🚀 Starting A2A Integration Test Suite (Rust Runner)"
            .blue()
            .bold()
    );
    ...
    // Manually handle agent card fetching outside run_test to get the value
    {
        ...
        println!(
            "{}",
            format!("--> [Test {}] Running: Get Agent Card...", test_num).yellow()
        );
        ...
        match tokio::time::timeout(config.default_timeout, get_card_closure()).await {
            Ok(Ok(card)) => {
                println!(
                    "    {}",
                    format!("✅ [Test {}] Success: Get Agent Card", test_num).green()
                );
                ...
                if let Some(caps) = agent_card_opt.as_ref().map(|c| &c.capabilities) {
                    ...
                    println!(
                        "    Agent Capabilities: Streaming={}, Push={}, History={}",
            ...
            Ok(Err(e)) => {
                println!(
                    "    {}",
                    format!(
                        "⚠️ [Test {}] Unsupported or not working (Error: {}): Get Agent Card",
                        test_num, e
                    )
                    .yellow()
                );
                ...
                println!(
                    "{}",
                    "⚠️ Could not retrieve Agent Card. Assuming default capabilities (false)."
                        .yellow()
                );
            }
            Err(_) => {
                println!(
                    "    {}",
                    format!(
                        "⚠️ [Test {}] Unsupported or not working (Timeout): Get Agent Card",
                        test_num
                    )
                    .yellow()
                );
    ...
    // Send Task (needs special handling to capture ID)
    {
        ...
        println!(
            "{}",
            format!("--> [Test {}] Running: Send Task...", test_num).yellow()
        );
        ...
        let send_task_closure = || {
            ...
            async move {
                ...
                client_guard
                    .send_task("This is a test task from Rust runner", None)
        ...
        match tokio::time::timeout(config.default_timeout, send_task_closure()).await {
            ...
            Ok(Err(e)) => {
                println!(
                    "    {}",
                    format!(
                        "⚠️ [Test {}] Unsupported or not working (Error: {}): Send Task",
                        test_num, e
                    )
                    .yellow()
                );
            ...
            Err(_) => {
                println!(
                    "    {}",
                    format!(
                        "⚠️ [Test {}] Unsupported or not working (Timeout): Send Task",
                        test_num
                    )
                    .yellow()
                );
    ...
    if let Some(ref id) = task_id {
        ...
        run_test(&results, "Get Task Details", false, &config, || {
            let client_clone = Arc::clone(&client_arc);
        ...
        run_test(&results, "Cancel Task", false, &config, || {
            ...
            async move {
                ...
                client_guard
                    .send_jsonrpc::<serde_json::Value>("tasks/cancel", params_value)
                    .await?;
                Ok(())
            }
        })
        ...
        println!(
            "{}",
            "⚠️ Skipping tests dependent on successful task creation.".yellow()
        );
    }
    ...
    if capabilities.streaming {
        ...
        run_test(&results, "Start Streaming Task", false, &config, || {
            let client_clone = Arc::clone(&client_arc);
            async move {
                let mut client_guard = client_clone.lock().await; // Use async lock
        ...
        println!(
            "{}",
            "⏭️ Skipping Streaming tests (Agent capability not reported).".blue()
        );
    ...
    if capabilities.push_notifications {
        ...
        if let Some(ref id) = task_id {
            ...
            run_test(&results, "Set Push Notification", false, &config, || {
                let client_clone = Arc::clone(&client_arc);
            ...
            run_test(&results, "Get Push Notification", false, &config, || {
                let client_clone = Arc::clone(&client_arc);
            ...
            println!(
                "{}",
                "⚠️ Skipping Push Notification tests as TASK_ID was not set.".yellow()
            );
        }
    } else {
        println!(
            "{}",
            "⏭️ Skipping Push Notification tests (Agent capability not reported).".blue()
        );
    ...
    if get_task_err_result.is_ok() {
        println!(
            "    {}",
            "❌ Test Failed: Expected an error but got Ok".red()
        );
        results.lock().unwrap().failure_count += 1;
        ...
        if !e.is_task_not_found() {
            println!(
                "    {}",
                format!(
                    "❌ Test Failed: Expected TaskNotFound error but got code {}",
                    e.code
                )
                .red()
            );
    ...
    );
    {
        ...
        println!(
            ...
            final_results.test_counter.to_string().bold()
        );
        println!(
            "{} {}",
            "Successful Tests:".green(),
            final_results.success_count.to_string().bold().green()
        );
        println!(
            "{} {}",
            "Unsupported/Not Working:".yellow(),
            final_results.failure_count.to_string().bold().yellow()
        );
        println!(
            "{} {}",
            "Skipped Tests:".blue(),
            final_results.skipped_count.to_string().bold().blue()
        );
...
impl Drop for MockServerGuard {
    fn drop(&mut self) {
        ...
        println!(
            "{}",
            "🛑 Local mock server will exit when tests complete.".yellow()
        );
    }
}
...
async fn start_mock_server_if_needed(
    config: &TestRunnerConfig,
) -> Result<(MockServerGuard, String), Box<dyn Error>> {
    if let Some(url) = &config.target_url {
        ...
        println!(
            "{} {}",
            "🚀 Starting local mock server on port".blue().bold(),
            port.to_string().cyan()
        );
